...
class provider implements \core_privacy\local\metadata\provider, \core_privacy\local\request\plugin\provider {
    ...
    public static function get_metadata(collection $collection) : collection {
        ...
        $collection->add_database_table(
            'pdfannotator_comments_archiv',
             [
                'userid' => 'privacy:metadata:pdfannotator_comments:userid',
                'annotationid' => 'privacy:metadata:pdfannotator_comments:annotationid',
                'content' => 'privacy:metadata:pdfannotator_comments:content',
             ],
            'privacy:metadata:pdfannotator_comments_archiv'
        );
    ...
    public static function get_contexts_for_userid(int $userid) : \core_privacy\local\request\contextlist {
        ...
        $sql = "SELECT DISTINCT c.id
                ...
                LEFT JOIN {pdfannotator_comments_archiv} ka ON ka.annotationid = a.id
...
//                    WHERE c.isquestion = 1 AND c.userid = :userid AND a.pdfannotatorid = :pdfannotator";
            ...
            foreach($myquestions as $myquestion) {
                $myquestion->timecreated = get_user_date_time($myquestion->timecreated);
            }
            ...
            $sql2 = "SELECT c.content, c.timecreated, c.visibility, q.content as questioncontent
                    FROM {pdfannotator_comments} c JOIN {pdfannotator_annotations} a ON c.annotationid = a.id JOIN {pdfannotator_comments} q ON q.annotationid = c.annotationid
            ...
            foreach($mycomments as $mycomment) {
                $mycomment->timecreated = get_user_date_time($mycomment->timecreated);
            }
...
//            $sql5 = "SELECT c.*
//                    FROM {pdfannotator_comments_archiv} c JOIN {pdfannotator_annotations} a ON c.annotationid = a.id
...
//            
            ...
            foreach($myDrawingsAndTextboxes as $myDrawingOrTextbox) {
                $myDrawingOrTextbox->timecreated = get_user_date_time($myDrawingOrTextbox->timecreated);
            }
    ...
    */
    public static function delete_data_for_all_users_in_context(\context $context) {
        ...
        $votes = $DB->get_records_sql("SELECT v.id FROM {pdfannotator_votes} v WHERE v.commentid IN (SELECT c.id FROM {pdfannotator_comments} c JOIN {pdfannotator_annotations} a ON c.annotationid = a.id WHERE a.pdfannotatorid = ?)", array($instanceid));
        foreach ($votes as $vote) {
            $DB->delete_records('pdfannotator_votes', array("id" => $vote->id));
        }
        
        // 3. Delete all subscriptions in this annotator
        $subscriptions = $DB->get_records_sql("SELECT s.id FROM {pdfannotator_subscriptions} s WHERE s.annotationid IN (SELECT a.id FROM {pdfannotator_annotations} a WHERE a.pdfannotatorid = ?)", array($instanceid));
        foreach ($subscriptions as $subscription) {
            $DB->delete_records('pdfannotator_subscriptions', array("id" => $subscription->id));
        }
        
        // 4. Delete all comments in this annotator
//        $comments = $DB->get_records_sql("SELECT c.id, c.isquestion, c.annotationid FROM {pdfannotator_comments} c WHERE c.annotationid IN (SELECT a.id FROM {pdfannotator_annotations} a WHERE a.pdfannotatorid = ?)", array($instanceid));
        $comments = $DB->get_records_sql("SELECT c.id FROM {pdfannotator_comments} c WHERE c.annotationid IN (SELECT a.id FROM {pdfannotator_annotations} a WHERE a.pdfannotatorid = ?)", array($instanceid));
...
//        $archivedcomments = $DB->get_records_sql("SELECT c.id, c.isquestion FROM {pdfannotator_comments_archiv} c WHERE c.annotationid IN (SELECT a.id FROM {pdfannotator_annotations} a WHERE a.pdfannotatorid = ?)", array($instanceid));
...
//            } else { // empty all other comments
//                $DB->update_record('pdfannotator_comments_archiv', array("id" => $archivedcomment->id, "userid" => -1, "content" => "", "isdeleted" => 1), $bulk=false);
//            }
//        }
...
//        require_once($CFG->dirroot.'/mod/pdfannotator/model/annotation.class.php');
        ...
        foreach ($contextlist->get_contexts() as $context) {
            ...
            $votes = $DB->get_records_sql("SELECT v.id FROM {pdfannotator_votes} v WHERE v.userid = ? AND v.commentid IN (SELECT c.id FROM {pdfannotator_comments} c WHERE c.pdfannotatorid = ?)", array($userid, $instanceid));
//            $votes = $DB->get_records_sql("SELECT v.id FROM {pdfannotator_votes} v WHERE v.userid = ? AND v.commentid IN (SELECT c.id FROM {pdfannotator_comments} c JOIN {pdfannotator_annotations} a ON c.annotationid = a.id WHERE a.pdfannotatorid = ?)", array($userid, $instanceid));
            foreach ($votes as $vote) {
                $DB->delete_records('pdfannotator_votes', array("id" => $vote->id));
            }
            
            // 3. Delete all subscriptions this user made in this annotator
            $subscriptions = $DB->get_records_sql("SELECT s.id FROM {pdfannotator_subscriptions} s WHERE s.userid = ? AND s.annotationid IN (SELECT a.id FROM {pdfannotator_annotations} a WHERE a.pdfannotatorid = ?)", array($userid, $instanceid));
            foreach ($subscriptions as $subscription) {
                $DB->delete_records('pdfannotator_subscriptions', array("id" => $subscription->id));
            }
...
//            $comments = $DB->get_records_sql("SELECT c.* FROM {pdfannotator_comments} c WHERE c.annotationid IN (SELECT a.id FROM {pdfannotator_annotations} a WHERE a.pdfannotatorid = ?) AND c.userid = ?", array($instanceid, $userid));
            foreach($comments as $comment) {
            ...
            // 5. Select the ID and isquestion attributes of every archived comment this user made in this annotator
//            $archivedcomments = $DB->get_records_sql("SELECT c.id, c.isquestion FROM {pdfannotator_comments_archiv} c WHERE c.annotationid IN (SELECT a.id FROM {pdfannotator_annotations} a WHERE a.pdfannotatorid = ?) AND c.userid = ?", array($instanceid, $userid));
//            foreach($archivedcomments as $archivedcomment) {
//                 if ($archivedcomment->isquestion === 1) { // delete question comments, their underlying annotation as well as all answers and subscriptions
//                    annotation::delete($archivedcomment->annotationid, null, true);
//                } else { // empty all other comments
...
//                $DB->delete_records('pdfannotator_annotations', array("id" => $annotationid));
    ...
    // Status quo:
    // Fragen und letzte Antworten werden beim Löschen aus der comments-Tabelle entfernt und - falls eine Meldung vorausging - in die Archivtabelle übernommen.
    // Mittlere Antworten werden beim Löschen nicht aus der comments-Tabelle entfernt. Stattdessen wird das Feld isdeleted auf 1 gesetzt. Falls eine Meldung vorausging, werden sie zusätzlich in die Archivtabelle übernommen.
    // In der Archivtabelle sollte das Feld isdeleted standardmäßig auf 1 stehen, steht aber auf 0.
    ...
    public static function delete_annotation($annotationid) {
        ...
        foreach($comments as $comment){
            ...
            if($comment->isdeleted == 0 && $DB->record_exists('pdfannotator_reports', ['commentid' => $comment->id])){
                unset($comment->id);
                $DB->insert_record('pdfannotator_comments_archiv',$comment);
            }
        }
    ...
    public static function empty_or_delete_comment($comment) {
        ...
        $wasanswered = $DB->record_exists_select('pdfannotator_comments', "annotationid = ? AND timecreated > ? AND isdeleted = ?", array($comment->annotationid, $comment->timecreated, 0));
